        ...
        <article className=" my-4  h-[1000px] scroll-thin  min-w-[1000px] overflow-x-auto scroll-thin">
          ...
          {users?.users?.map((u) => (
            <TR className=" ">
              ...
              <TH>{u?.phoneNumber}</TH>
              <TH>{wilayas[Number(u?.state) - 1 || 1]}</TH>
              <TH>{u?.isAdmin ? "admin" : u?.role || "normal"}</TH>
              <TH>
          ...
          <EditUserModal modal={modal} setmodal={setmodal} _id={id} />
          <ZrSend open={mdl} setopen={setmdl} o={{ user: { _id: id } }} />
        </article>
      )}
    </section>
  );
...
export const TR = ({ children, className }) => (
  <tr
    className={`break-all  grid grid-cols-[1fr_2.5fr_1fr_1.5fr_1fr_0.8fr_1fr] ${className}`}
  >
    {children}
  </tr>
);
